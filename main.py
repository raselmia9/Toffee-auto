...
async def process_single_channel(context, item):
    """প্রতিটি চ্যানেলের জন্য আলাদা ট্যাব বা পেজ ওপেন করে দ্রুত লিংক বের করার ফাংশন"""
    stream_link = ""
    try:
        ...
        try:
            await new_page.wait_for_selector("video", timeout=6000)
            if not stream_link:
...
async def generate_proper_playlist():
    ...
    async with async_playwright() as p:
        ...
        print(f"মোট {len(channels_info)} টি চ্যানেল পাওয়া গেছে। মাল্টি-ট্যাবে স্ট্রিম লিংক ক্যাপচার করা হচ্ছে...")

        # একসাথে একাধিক (যেমন ৩টি করে) ট্যাব চালিয়ে দ্রুত লিংক তোলার ব্যবস্থা
        chunk_size = 3
        final_playlist_data = []
        
        for i in range(0, len(channels_info), chunk_size):
            chunk = channels_info[i:i + chunk_size]
            tasks = [process_single_channel(context, item) for item in chunk]
